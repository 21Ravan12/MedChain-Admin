        ...
        <div class="verify-patients">
          ...
          <div class="page-header">
            ...
            <div class="header-controls">
              ...
              <select v-model="filterStatus" class="status-filter" @change="fetchPatients">
          ...
          <div v-if="error" class="error-alert">
            ...
            <span>{{ error }}</span>
            <button @click="fetchPatients" class="btn-retry">
            ...
            >
              <div class="patient-info">
                ...
                <div class="details">
                  <div class="name-demographics">
                    <h3>{{ patient.fullName || patient.name }}</h3>
                  ...
                  <div class="patient-meta">
                    <div class="contact-info">
                      <p><i class="fas fa-envelope"></i> {{ patient.email || 'No email' }}</p>
                      <p><i class="fas fa-id-card"></i> ID: {{ patient.patientId || patient.adminId }}</p>
                    </div>
                    <div class="submission-info">
                      <div class="submission-date">
                        <i class="fas fa-calendar-alt"></i>
                        Submitted: {{ formatDate(patient.submittedAt) }}
                      </div>
                      <div v-if="patient.reviewedAt" class="review-date">
                        <i class="fas fa-check-circle"></i>
                        Reviewed: {{ formatDate(patient.reviewedAt) }}
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div class="verification-actions" @click.stop>
                <button 
                  ...
                  @click.stop="approvePatient(patient.id)" 
                  class="btn btn-success"
                  :disabled="processingApproval === patient.id"
                >
                  <i class="fas fa-check"></i> 
                  <span>{{ processingApproval === patient.id ? 'Processing...' : 'Approve' }}</span>
                </button>
                <button 
                  v-if="patient.status === 'pending'" 
                  @click.stop="showRejectModal(patient.id)" 
                  class="btn btn-danger"
                  :disabled="processingRejection === patient.id"
                >
                  <i class="fas fa-times"></i> 
                  <span>{{ processingRejection === patient.id ? 'Processing...' : 'Reject' }}</span>
                </button>
                <button 
...
<script>
import RejectModal from '@/components/admin/verify/patients/RejectModal.vue';
import PatientDetailModal from '@/components/admin/verify/patients/PatientDetailModal.vue';
...
export default {
    ...
    filteredPatients() {
      ...
      if (this.searchQuery) {
        ...
        filtered = filtered.filter(p => 
          (p.fullName || p.name).toLowerCase().includes(query) ||
          p.email.toLowerCase().includes(query) ||
          (p.patientId || p.adminId).toLowerCase().includes(query)
        );
      }
  ...
  created() {
    this.fetchPatientAdmins();
    this.debouncedSearch = debounce(this.fetchPatientAdmins, 500);
  },
  methods: {
    async fetchPatientAdmins() {
      this.loading = true;
      this.error = null;
      try {
        const response = await this.$store.dispatch('admin/fetchUnverifiedPatients', {
        ...
        this.patients = response.data.data.map(admin => {
          ...
          return {
            ...
            patientName: admin.patient_name || 'Not specified',
            email: admin.email,
            phone: admin.phone || 'Not provided',
            age: admin.birthyear,
            adminId: admin.id_proof || 'Not provided',
        ...
        this.error = error.response?.data?.message || error.message || 'Failed to load patients';
    ...
    async approvePatient(patientId) {
      this.processingApproval = patientId;
      try {
        ...
        await this.fetchPatientAdmins();
        this.$toast.success('Patient approved successfully');
      } catch (error) {
        const errorMessage = error?.response?.data?.message 
          || error?.message 
          || 'An unexpected error occurred while approving the patient.';
    ...
    showRejectModal(id) {
      this.patientToReject = id;
      this.showRejectReasonModal = true;
    },
    
    async rejectPatient(rejectionReason) {
      if (!rejectionReason?.trim()) {
        this.$toast.error('Rejection reason is required');
        return false;
      }

      if (!this.patientToReject) {
        this.$toast.error('No patient selected for rejection');
        return false;
      }

      this.processingRejection = this.patientToReject;
      this.showRejectReasonModal = false;

      try {
        await this.$store.dispatch('admin/reject', {
          entityId: this.patientToReject,
          entityType: 'patient',  
          description: rejectionReason.trim()
        });

        await this.fetchPatientAdmins();
        this.$toast.success('Patient rejected successfully');
        return true;
      } catch (error) {
        console.error('Patient rejection failed:', error);
        
        const errorMessage = error.response?.data?.message 
          || error.message 
          || 'An error occurred while rejecting the patient';
        
        this.$toast.error(errorMessage);
        return false;
      } finally {
        this.patientToReject = null;
