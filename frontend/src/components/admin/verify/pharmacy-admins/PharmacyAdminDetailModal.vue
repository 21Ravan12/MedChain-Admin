  ...
  <div class="admin-detail-modal">
    ...
    <div class="modal-content">
      ...
      <div class="admin-details">
        ...
        <div class="detail-section" v-if="admin.documents && admin.documents.length">
          <div class="section-header">
            <h3><i class="fas fa-file-alt"></i> Pharmacy License Documents</h3>
            <span class="badge">{{ admin.documents.length }}</span>
          </div>
          <div class="document-list">
            <div v-for="(doc, index) in admin.documents" :key="index" class="document-item">
      ...
      <div class="modal-footer" v-if="admin.status === 'pending'">
        <button class="btn btn-success" @click="$emit('approve', admin.id)">
          <i class="fas fa-check"></i> Approve Pharmacy Admin
        </button>
        <button class="btn btn-danger" @click="$emit('reject', admin.id)">
...
export default {
  ...
  methods: {
    getDocumentIcon(doc) {
      // Handle case where doc might be an object
      const filename = typeof doc === 'object' ? doc.name || doc.filename : doc;
    ...
    getDocumentName(doc) {
      // Handle case where doc might be an object
      return typeof doc === 'object' ? doc.name || doc.filename || 'Document' : doc;
    },
    getDocumentSize(doc) {
      // Handle case where doc might be an object with size property
      if (typeof doc === 'object' && doc.size) {
    ...
    downloadProfile() {
      console.log('Downloading pharmacy admin profile');
      // Implement profile download logic here
    },
